...
net.to(device)
# test_kmean_accuracy(net.backbone, DataLoader(
    # test_dataset, batch_size=config['batch_size']), device)
net.train()
...
for epoch in range(int(config['epochs'])):
    ...
    if (epoch + 1) % int(config['model_save_freq']) == 0:
        save_model(epoch, net, opt, training_loss_values,
                   validation_loss_values, config['batch_size'], config['checkpoint_dir'], config['opt'])
        test_kmean_accuracy(net.backbone, DataLoader(
            test_dataset, batch_size=config['batch_size']), device)
